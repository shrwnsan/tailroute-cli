...
# Probes the SOCKS5 proxy with a real CONNECT to the actual target before use,
# delegating the handshake to nc's own SOCKS5 client (-X 5 -x).  The probe
# exercises the exact code path of the real connection, so its exit code is
# the ground truth: proxy running, speaking SOCKS5, and able to reach the
# target.  Falls back to direct otherwise.  Timeout: 3 seconds (T-433).
#
# Do not hand-edit: regenerate with `tailroute proxy-config ssh --replace-wrapper`.
# Flag note: macOS /usr/bin/nc has no -q; only -X/-x/-z/-w are used (verified).
...
if /usr/bin/nc -X 5 -x "${PROXY_ADDR}:${PROXY_PORT}" -z -w "$TIMEOUT" "$HOST" "$PORT" >/dev/null 2>&1; then
    exec /usr/bin/nc -X 5 -x "${PROXY_ADDR}:${PROXY_PORT}" "$@"
fi
exec /usr/bin/nc "$@"
EOF
}
